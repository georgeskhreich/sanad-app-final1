...
// فحص ما إذا كان المستخدم قد أدخل مفاتيح Firebase الحقيقية
...
{ id: "attic", label: "سقيفة أو مخزن خارجي مستقل", icon: "📦" },
...
// ==========================================
// فحص وتشغيل نظام المزامنة والربط السحابي
// ==========================================
useEffect(() => {
if (!isFirebaseConfigured || !auth) {
// إذا لم يكن الفايربيس مهيأ، يتم استخدام الذاكرة المحلية فوراً (Fail-Safe)
const cachedSurveys = localStorage.getItem('ain-ebel-surveys');
const cachedUsers = localStorage.getItem('ain-ebel-users');

  if (cachedSurveys) {
    setSurveys(JSON.parse(cachedSurveys));
  } else {
    setSurveys(SEED_SURVEYS);
    localStorage.setItem('ain-ebel-surveys', JSON.stringify(SEED_SURVEYS));
  }
  ...
  if (cachedUsers) {
    ...
    localStorage.setItem('ain-ebel-users', JSON.stringify(SEED_USERS));
  }
  return;
}

// تسجيل دخول مجهول آمن للفايربيس
signInAnonymously(auth)
  .then((userCredential) => setFirebaseUser(userCredential.user))
  .catch(err => console.error("Firebase Anonymous Auth failed:", err));


}, []);

// جلب البيانات من السيرفر السحابي لحظة بلحظة
useEffect(() => {
if (!isFirebaseConfigured || !firebaseUser || !db) return;

// استيراد آمن مع رصد الأخطاء لمنع ظهور الشاشة البيضاء
...
const unsubscribeSurveys = onSnapshot(surveysColRef, (snapshot) => {
  ...
  console.error("Surveys Firestore Error:", error);
  showToast("خطأ بالاتصال بالسحابة لقراءة الاستمارات.");
});
...
// تهيئة محلية في حال عدم توفر سحابة
setSurveys(SEED_SURVEYS);
setUsersList(SEED_USERS);
localStorage.setItem('ain-ebel-surveys', JSON.stringify(SEED_SURVEYS));
localStorage.setItem('ain-ebel-users', JSON.stringify(SEED_USERS));
showToast("تم تهيئة وحفظ مستندات عين إبل محلياً بالهاتف بنجاح!");
return;
}
try {
...
if (isOnline && db) {
  ...
  // حفظ محلي أمن
  const newList = [{ id: `DRAFT-${Date.now()}`, ...newSurveyPayload }, ...drafts];
  setDrafts(newList);
  setSurveys([{ id: `DRAFT-${Date.now()}`, ...newSurveyPayload }, ...surveys]);
  localStorage.setItem('ain-ebel-surveys', JSON.stringify([{ id: `DRAFT-${Date.now()}`, ...newSurveyPayload }, ...surveys]));
  showToast("تم حفظ التقرير محلياً بنجاح في الهاتف.");
}
...
if (isOnline && db) {
  try {
    await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'users'), newUserPayload);
    showToast(`تم إنشاء حساب ${role === "Supervisor" ? "مشرف" : "مهندس"} بنجاح.`);
  } catch (err) {
    showToast("فشل إنشاء الحساب السحابي.");
  }
} else {
  const newList = [{ id: `u-${Date.now()}`, ...newUserPayload }, ...usersList];
  setUsersList(newList);
  localStorage.setItem('ain-ebel-users', JSON.stringify(newList));
  showToast(`تم إنشاء حساب ${role === "Supervisor" ? "مشرف" : "مهندس"} محلياً بنجاح.`);
}
...
showToast("تم تحديث كلمة المرور سحابياً بنجاح.");
} catch (err) {
showToast("خطأ سحابي في التعديل.");
}
} else {
const newList = usersList.map(u => u.id === editingUser.id ? { ...u, password: editingUser.newPassword } : u);
setUsersList(newList);
localStorage.setItem('ain-ebel-users', JSON.stringify(newList));
...
setUsersList(newList);
localStorage.setItem('ain-ebel-users', JSON.stringify(newList));
showToast("تم حذف الحساب محلياً بنجاح.");
}
};
...
showToast("تم حذف التقرير نهائياً.");
} catch (e) {
showToast("خطأ سحابي في الحذف.");
}
} else {
...
setSurveys(newList);
localStorage.setItem('ain-ebel-surveys', JSON.stringify(newList));
setSelectedSurvey(null);
showToast("تم حذف التقرير محلياً بنجاح.");
}
};
...
showToast("فشل تحديث الحالة سحابياً.");
}
} else {
const newList = surveys.map(s => s.id === id ? { ...s, status } : s);
setSurveys(newList);
localStorage.setItem('ain-ebel-surveys', JSON.stringify(newList));
if (selectedSurvey && selectedSurvey.id === id) {
setSelectedSurvey(prev => ({ ...prev, status }));
...
const sId = s.id || "";
const matchesSearch = name.toLowerCase().includes(searchTerm.toLowerCase()) || sId.toLowerCase().includes(searchTerm.toLowerCase());
                  ...
                  <div className="bg-slate-50 p-4 rounded-xl border space-y-4">
                    ...
                    {[
                      ...
                      <div key={item.k} className="bg-white p-3 rounded-lg border flex flex-col sm:flex-row justify-between gap-3 text-xs">
                        ...
                        <select className="p-1 border rounded-lg bg-slate-50 text-[10px]" value={formData[`furniture_${item.key}_damage`]} onChange={(e)=>setFormData({...formData, [`furniture_${item.key}_damage`]:e.target.value})}>
                ...
                <div className="xl:col-span-5 space-y-4">
                  ...
                  <div className="space-y-2 max-h-[350px] overflow-y-auto pr-1 text-xs">
                    {filteredSurveys.map(s => {
                      const surveyId = s.id || "";
                      const ownerName = s.ownerName || "غير محدد";
                      const engineerName = s.engineerName || "غير معروف";
                      return (
                        <div key={surveyId} onClick={()=>setSelectedSurvey(s)} className={`p-3 border rounded-xl cursor-pointer ${selectedSurvey?.id === s.id ? "border-emerald-600 bg-emerald-50/40" : "border-slate-200 hover:bg-slate-50"}`}>
                          <div className="flex justify-between items-center"><span className="font-mono text-[9px] text-slate-400">{surveyId.substring(0,8)}</span><span className={`text-[9px] px-2 py-0.5 rounded font-bold ${s.severity === "جسيم / خطر" ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-800"}`}>{s.severity}</span></div>
                          <h4 className="font-bold mt-1">{ownerName}</h4>
                          <div className="flex justify-between mt-2 pt-2 border-t text-[10px] text-slate-400"><span>م. {engineerName}</span><strong>{s.status}</strong></div>
                        </div>
                      );
                    })}
                  </div>
